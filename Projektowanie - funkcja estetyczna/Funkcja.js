...
function znajdzSymetrie(obrazek, polKwadr, polTrojk, polKol)
{
	...
	if(wartoscPodobienstwa/pelnaSymetria>0.8)
...
function oceń(obrazek)
{
	...
	var punktyZaNachodzenieNaSiebie=-5;
	...
	wynik+=sumaPunktowZaRoznorodnosc;
...
function ekranOceny()
{
	var strona=d3.select("body").append("div").attr("id","strona")
	var obrazekIOcena=strona.append("div").attr("id","gora")
	var legenda=strona.append("div").attr("id","dol").style("clear","both")
	...
	var obraz=obrazekIOcena.append("div").attr("id","lewo").style("width","80%")
		.style("float","left")
	var interfejsOceny=obrazekIOcena.append("div").attr("id","prawo").style("width","20%")
		.style("float","left")
	interfejsOceny=interfejsOceny.append("fieldset")
	...
	function klikniecie(e){
		...
		var wyniki=[];
		for(i=0;i<doOceny.length;i++)
		{
			var u=i;
			wyniki.push(oceń(doOceny[i]));
